...
void StatusDate ::addOrUpdateStatus(vector<Employee*> list){
    ...
    ifstream ifs("C:\\Users\\TCC\\Desktop\\fileNV\\"+idNV+".csv", ios::in);
    ...
    int n=Helpper ::numberLine("C:\\Users\\TCC\\Desktop\\fileNV\\"+idNV+".csv"); // lấy ra số dòng
    for (int i=0 ; i < n ; i++){
        char lines[500];
        fflush(stdin);
...
void StatusDate::writeRollCall(map<string,string> mapRollCall ,string idNV){
    ofstream fstream_ob;
    fstream_ob.open("C:\\Users\\TCC\\Desktop\\fileNV\\"+idNV+".csv", ios::trunc);
...
void StatusDate::addOrUpdateStatus1(map<std::string, std::string> mapRollCall ,string idNV){
    ...
    fstream output("C:\\Users\\TCC\\Desktop\\fileNV\\"+idNV+".csv", ios::app);
